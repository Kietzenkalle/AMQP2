...
public class RPCRequester implements Callable{
	...
	public Object call() throws Exception {
		...
		channel.basicPublish(server.SERVER_NAME+"."+target, server.trustedClouds.get(target)[0], properties, SerializationUtils.serialize(fullMessage));
		System.out.println("Message: '"+ message+ "' sent from '" + server.SERVER_NAME +"' to '"+ target +"' over exchange: "+server.SERVER_NAME+"."+target+" queue '" + server.trustedClouds.get(target)[0]);
		QueueingConsumer consumer = new QueueingConsumer(channel);
		channel.basicConsume(server.trustedClouds.get(target)[2], false, consumer);
